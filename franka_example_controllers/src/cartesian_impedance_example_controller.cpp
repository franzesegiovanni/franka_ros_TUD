...
void CartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  std::array< double, 3 > gravity_earth={{0., 0.,-9.81}};

  // F_x_Ctotal[2]=0;
  std::array<double, 7> gravity_old_array = model_handle_->getGravity();
  std::array<double, 7> gravity_new_array = model_handle_->getGravity(gravity_earth);
  ...
  tau_d << tau_task + tau_nullspace + coriolis+ tau_joint_limit-gravity_old+gravity_new;
